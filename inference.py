...
from torchvision import transforms, datasets # 新增 datasets
from models.resnet50 import get_resnet50, get_se_resnet50, get_finegrained_resnet50

def get_args():
    parser = argparse.ArgumentParser(description="Run inference on test set.")
    parser.add_argument("--exp_name", type=str, required=True, help="Experiment name (folder in checkpoints/).")
    ...
    parser.add_argument("--output_csv", type=str, default="prediction.csv", help="Output CSV file path.")
    parser.add_argument("--use_tta", action="store_true", help="Enable test-time augmentation (multi-scale + hflip).")
    parser.add_argument("--tta_scales", type=str, default="1.0,1.1", help="Comma-separated scales for TTA, e.g. '1.0,1.1'.")
...
def predict():
    ...
    model_path = os.path.join("checkpoints", args.exp_name, "best_model.pth")
    ...
    if args.use_tta:
        ...
        print(f"TTA enabled. scales={scales}, views_per_image={len(tta_transforms) * 2}")
    else:
