...
from models.resnet50 import get_finegrained_resnet50, get_se_resnet50
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train a ResNet50 model on the given dataset."
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Use Mixup or CutMix data augmentation.",
    )
    parser.add_argument(
        "--mix_alpha", type=float, default=0.9, help="Alpha value for Mixup/CutMix."
    )
    parser.add_argument(
        "--transform_type",
        type=str,
        default="manual",
        choices=["AA", "manual"],
        help="Augmentation strategy: AA (AutoAugment) or manual.",
    )
    parser.add_argument(
        "--model_type",
        type=str,
        default="se",
        choices=["se", "fg_gem"],
        help="Model architecture: se or fg_gem.",
    )
    parser.add_argument(
        "--dropout", type=float, default=0.5, help="Dropout rate for fg_gem head."
    )
    parser.add_argument(
        "--gem_p", type=float, default=3.0, help="Initial GeM p for fg_gem model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Strength of weighted sampling. 1.0 means inverse-frequency.",
    )
...
def train():
    ...
    if args.model_type == "fg_gem":
        model = get_finegrained_resnet50(dropout=args.dropout, gem_p=args.gem_p).to(
            device
        )
    else:
    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, (images, labels) in pbar:
            ...
            if do_mix:
                loss = mix_ratio * criterion(outputs, labels_a) + (
                    1 - mix_ratio
        ...
        print(
            f"Epoch {epoch+1} Summary: Val Acc {val_acc:.2f}%, Val Loss {val_loss:.4f}"
        )
